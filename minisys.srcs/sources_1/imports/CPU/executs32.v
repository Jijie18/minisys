...
module Executs32(Read_data_1,Read_data_2,Sign_extend,Function_opcode,Exe_opcode,ALUOp,
    ...
    input[31:0]  Read_data_1;		// ??????Read_data_1??
    input[31:0]  Read_data_2;		// ??????Read_data_2??
    input[31:0]  Sign_extend;		// ??????????????
    input[5:0]   Function_opcode;  	// ??????r-???????,r-form instructions[5:0]
    input[5:0]   Exe_opcode;  		// ?????????
    input[1:0]   ALUOp;             // ???????????????
    input[4:0]   Shamt;             // ???????instruction[10:6]???????
    input  		 Sftmd;            // ???????????????
    input        ALUSrc;            // ????????????????????beq?bne???
    input        I_format;          // ???????????beq, bne, LW, SW???I-????
    input        Jrn;               // ??????????JR??
    output       Zero;              // ?1??????0 
...
always @* begin  // 6?????
   ...
   always @* begin
            ...
            else if((ALU_ctl==3'b101) && (I_format==1)) ALU_Result[31:0] = Binput<<16  ;   //lui data
    ...
    assign Add_Result = Branch_Add[31:0];   //??????PC??????4?????????16?
